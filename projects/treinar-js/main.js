...
async function carregarDesafio(id) {
  ...
  const path = `js/desafio-de-codigo-${id}.js`;
  nexusState.currentChallenge = id;

  // A. Feedback Visual na Sidebar (Seleção)
  document
  ...
  document.getElementById("source-name").innerText =
    `desafio-de-codigo-${id}.js`;
  document.getElementById("view-title").innerText =
    `DESAFIO ${id.toString().padStart(2, "0")}`;
  ...
  // D. Injeção Dinâmica do Script (Execução)
  const oldScript = document.getElementById("dynamic-challenge-script");
  if (oldScript) oldScript.remove();

  const script = document.createElement("script");
  script.id = "dynamic-challenge-script";
  script.src = path;

  script.onload = () => {
    imprimirNoTerminal(`// Módulo ${id} pronto para execução.`, "system-msg");
    document.getElementById("btn-run").disabled = false;
    atualizarInspector(id); // Sincroniza a documentação na direita
  };

  script.onerror = () => {
    imprimirNoTerminal(`[ERRO] Falha ao injetar script: ${path}`, "error");
  };
