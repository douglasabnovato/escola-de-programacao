...
// --- 2. Motor de Carregamento e Interface (Ajustado para Títulos Dinâmicos) ---
async function carregarDesafio(id) {
  if (nexusState.isExecuting) return;

  // Garante que o ID seja uma string de 2 dígitos para bater com o aulas_config.js
  const idFormatado = id.toString().padStart(2, "0");
  
  // Busca o título no nosso objeto de configuração
  // Se não encontrar (ex: aula nova ainda não mapeada), usa um fallback
  const tituloAula = titulosAulas[idFormatado] || `Aula ${idFormatado}`;

  // Ajuste do path conforme sua estrutura
  const path = `aulas/aula${id}/index.html`;
  nexusState.currentChallenge = id;

  // A. Feedback Visual na Sidebar
  document
    .querySelectorAll(".nav-item") // Certifique-se que seus links de aula tenham essa classe
    .forEach((el) => el.classList.remove("active"));
    
  const item = document.getElementById(`menu-desafio-${id}`);
  if (item) item.classList.add("active");

  // B. Atualização do Breadcrumb e Títulos (AQUI ESTÁ A MÁGICA)
  document.getElementById("source-name").innerText = `aula-${idFormatado}/index.html`;
  
  // Agora o título exibe o nome real da aula definido no config
  document.getElementById("view-title").innerText = `AULA ${idFormatado} - ${tituloAula}`;
  ...
  try {
    ...
    iframe.onload = () => {
      // Log no terminal agora também com o título da aula
      imprimirNoTerminal(`// ${tituloAula} carregada com sucesso.`, "system-msg");
      document.getElementById("btn-run").disabled = true;
      atualizarInspector(id);
    };
  } catch (err) {
    console.error(err);
    document.getElementById("code-content").innerText = `// Erro ao carregar fonte: ${path}`;
